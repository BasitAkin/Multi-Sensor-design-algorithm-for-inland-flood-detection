...
The input data files required to run the scripts are archived on Zenodo with a citable DOI. The `master_dataset.csv` is too large to host on GitHub (190 MB; GitHub's per-file limit is 100 MB), so download all data files from the Zenodo deposit before running the scripts.

**[Download the input dataset from Zenodo (DOI: 10.5281/zenodo.20389808)](https://doi.org/10.5281/zenodo.20389808)**

The Zenodo deposit is licensed under Creative Commons Attribution 4.0 International (CC-BY 4.0). After downloading, your `data/` folder should contain:

- `master_dataset.csv` - Candidate sensor locations with hydrogeomorphic and risk attributes
- `studyarea_huc10s.shp` (with associated `.dbf`, `.shx`, `.prj`, `.cpg`, `.sbn`, `.sbx`, `.shp.xml` components) - HUC10 watershed boundaries for the study area
- `USGSgages_in_ROI_5states.csv` - Active USGS streamgage locations within the study domain

*Optional mirror:* the same files are also available on Google Drive at [this link](https://drive.google.com/drive/folders/1vhDDwFzDJLjArZ8lZ5QxicHCu5aAzI46?usp=sharing) for convenience. The Zenodo deposit is the canonical, citable source.
...
Multi-Sensor Network Design Optimization for Inland Flood Detection (v1.0.0) [Computer software].
Zenodo. https://doi.org/10.5281/zenodo.20389783
```

> A `CITATION.cff` file in this repository enables GitHub's "Cite this repository" button (top-right of the repo page) and provides the same information in a machine-readable form.

### Input data

```
Akinade, B. A., Amanambu, A. C., & Davis, M. A. L. (2026). Input dataset for
"Where to Watch the Water: Multi-Sensor Network Design Optimization for Inland
Flood Detection" [Dataset]. Zenodo. https://doi.org/10.5281/zenodo.20389808
```
